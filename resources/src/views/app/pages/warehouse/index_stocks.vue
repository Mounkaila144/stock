<template>
  <div class="main-content">
    <breadcumb :page="$t('StockList')" :folder="$t('Warehouse')" />
      ...
      prevLabel: 'prev',
        ...
        <template slot="table-row" slot-scope="props">
          ...
          <span v-else-if="props.column.field == 'status'">
            <div v-if="props.row.type == 'is_service'">...</div>
            <div v-else>
              <!-- {{ { QTY: ExtractNumber(props.row.quantity) } }} -->

              <span v-if="(ExtractNumber(props.row.pending_purcahse_quantity)?? 0) > 0" class="badge badge-info">{{ $t('Expecting')
                }} <strong>{{ props.row.pending_purcahse_quantity }}</strong></span>

              <span v-else-if="(ExtractNumber(props.row.quantity)?? 0) <= 0" class="badge badge-danger">{{ $t('OutOfStock')
                }}</span>
              <span v-else-if="ExtractNumber(props.row.quantity) <= (props.row.stock_alert ?? 0)"
                class="badge badge-warning">{{ $t('StockAlert') }}</span>
              <span v-else class="badge badge-success">{{ $t('InStock') }}</span>
            </div>
          </span>
        </template>
      </vue-good-table>
