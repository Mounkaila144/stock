    ...
    <b-card v-if="!isLoading">
      <b-row>
        <b-col md="12" class="mb-5">
          <router-link v-if="currentUserPermissions && currentUserPermissions.includes('Quotations_edit')" title="Edit"
          ...
          <router-link
            ...
            title="Create" class="btn-sm btn btn-primary ripple btn-icon m-1"
          ...
          <a :href="`/api/quote_pdf/${$route.params.id}`" target="_blank" class="btn-sm btn btn-light ripple btn-icon m-1">
            <i class="i-File-TXT"></i> PDF
          </a>
          ...
          <button v-if="currentUserPermissions && currentUserPermissions.includes('Quotations_delete')"
            @click="Remove_Quote()" class="btn btn-danger btn-icon icon-left btn-sm m-1">
        ...
        <div class="invoice-print">
          ...
          <b-row class="mt-5">
            ...
            <b-col md="4" class="mb-4">
              ...
              <div>
                {{ $t('Status') }} :
                <span v-if="quote.statut == 'sent'" class="badge badge-outline-success">{{ $t('Sent') }}</span>
                  ...
                  <tbody>
                    <tr v-for="detail in details">
                      <td><span>{{ detail.code }} ({{ detail.name }})</span>
                        <p v-show="detail.is_imei && detail.imei_number !== null">{{ $t('IMEI_SN') }} :
                          {{ detail.imei_number }}</p>
                      </td>
              ...
              <table class="table table-striped table-sm">
                <tbody>
                  <tr>
                    ...
                    <td>
                      <span>{{ currentUser.currency }} {{ quote.TaxNet? quote.TaxNet.toFixed(2) : '' }} ({{ formatNumber(quote.tax_rate, 2) }}
                        %)</span>
                    </td>
                  </tr>
                  <tr>
                    <td class="bold">{{ $t('Discount') }}</td>
                    <td>{{ currentUser.currency }} {{ quote.discount? quote.discount.toFixed(2) : '' }}</td>
                  </tr>
                  <tr>
                    <td class="bold">{{ $t('Shipping') }}</td>
                    <td>{{ currentUser.currency }} {{ quote.shipping? quote.shipping.toFixed(2) :  '' }}</td>
                  </tr>
                  <tr>
                    <td>
                    ...
                    <td>
                      <span class="font-weight-bold">{{ currentUser.currency }} {{ quote.GrandTotal }}</span>
                    </td>
                  </tr>
                </tbody>
              </table>
