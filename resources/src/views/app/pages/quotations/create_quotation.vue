  ...
  <div class="main-content">
    ...
    <validation-observer ref="create_quote" v-if="!isLoading">
          ...
          <b-col lg="12" md="12" sm="12">
            <b-card>
              <b-row>
                  ...
                  <validation-provider name="date" :rules="{ required: true }" v-slot="validationContext">
                    <b-form-group :label="$t('date') + ' ' + '*'">
                      <b-form-input :state="getValidationState(validationContext)" aria-describedby="date-feedback"
                        type="date" v-model="quote.date"></b-form-input>
                  ...
                  <validation-provider name="Customer" :rules="{ required: true }">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Customer') + ' ' + '*'">
                      <v-select :class="{ 'is-invalid': !!errors.length }"
                        :state="errors[0] ? false : (valid ? true : null)" v-model="quote.client_id"
                  ...
                  <validation-provider name="warehouse" :rules="{ required: true }">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('warehouse') + ' ' + '*'">
                      <v-select :class="{ 'is-invalid': !!errors.length }"
                        ...
                        @input="Selected_Warehouse" v-model="quote.warehouse_id" :reduce="label => label.value"
                ...
                <b-col sm="12" md="1" class="pt-3">
                  <CreateFreetextDetail @submit="(data) => AddUnamed(data)" />
                </b-col>
                ...
                <b-col md="12">
                  <b-form-group :label="$t('OBJECT')">
                    <textarea v-model="quote.notes" rows="4" class="form-control"
                  ...
                  <table class="table table-striped table-sm">
                    <tbody>
                      <tr>
                        ...
                        <td>
                          <span>{{ currentUser.currency }} {{ quote.TaxNet.toFixed(2) }} ({{
      formatNumber(quote.tax_rate, 2) }}
                            %)</span>
                        </td>
                      </tr>
                      <tr>
                    ...
                    <b-form-group :label="$t('OrderTax')">
                      <b-input-group append="%">
                        <b-form-input :state="getValidationState(validationContext)"
                          aria-describedby="OrderTax-feedback" label="Order Tax" v-model.number="quote.tax_rate"
                  ...
                  <validation-provider name="Discount" :rules="{ regex: /^\d*\.?\d*$/ }" v-slot="validationContext">
                    <b-form-group :label="$t('Discount')">
                      <b-input-group :append="currentUser.currency">
                        <b-form-input :state="getValidationState(validationContext)"
                          aria-describedby="Discount-feedback" label="Discount" v-model.number="quote.discount"
                ...
                <b-col lg="4" md="4" sm="12" class="mb-3"
                  ...
                  <validation-provider name="Shipping" :rules="{ regex: /^\d*\.?\d*$/ }" v-slot="validationContext">
                    <b-form-group :label="$t('Shipping')">
                      <b-input-group :append="currentUser.currency">
                        <b-form-input :state="getValidationState(validationContext)"
                          aria-describedby="Shipping-feedback" label="Shipping" v-model.number="quote.shipping"
                ...
                <!-- <b-col lg="4" md="4" sm="12" class="mb-3">
                  <validation-provider name="Status" :rules="{ required: true }">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Status') + ' ' + '*'">
                      <v-select :class="{ 'is-invalid': !!errors.length }"
                        :state="errors[0] ? false : (valid ? true : null)" v-model="quote.statut"
      ...
      { label: 'Pending', value: 'pending' }
                ...
                <b-col md="12">
                  <b-form-group>
                    <b-button variant="primary" @click="Submit_Quotation" :disabled="SubmitProcessing"><i
    ...
    <validation-observer ref="Update_Detail_quote">
      ...
      { label: 'Fixed', value: '2' }
            ...
            <b-col lg="6" md="6" sm="12" v-if="detail.product_type != 'is_service'">
              <validation-provider name="Unit Sale" :rules="{ required: false }">
...
import CreateFreetextDetail from './create_freetext_detail.vue';

export default {
  components: { CreateFreetextDetail },
  metaInfo: {
  ...
  methods: {
    ...
    AddUnamed(data) {
      ...
      this.product = {};
      // if (result.product_type == 'is_service') {
      this.product.quantity = data?.quantity || 1;
      this.product.code = data?.description ?? "Free Text";//result.code;
      this.product.Unit_price = data?.unitPrice ?? 0;
      ...
      this.product.is_imei = false,//response.data.is_imei;
      ...
      axios
        ...
        .then(response => {
          ...
          if (!this.quote.warehouse_id) {
            this.quote.warehouse_id = response.data.warehouses[0]?.id;
            setTimeout(() => {
